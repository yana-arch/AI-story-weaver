...
export const KeywordPresetManager: React.FC<KeywordPresetManagerProps> = ({ presets, setPresets, onClose }) => {
                ...
                <div className="flex-grow overflow-y-auto pr-2">
                    <ul className="space-y-3">
                        {presets.length > 0 ? presets.map(preset => (
                            <li key={preset.id} className="p-3 rounded-md bg-gray-700/50 flex justify-between items-start gap-4">
                                {editingPreset?.id === preset.id ? (
                                    <div className="flex-grow space-y-2">
                                        ...
                                        <textarea
                                            ...
                                            onChange={(e) => setEditingPreset({...editingPreset, avoidKeywords: e.target.value})}
                                        ...
                                        <textarea
                                            ...
                                            onChange={(e) => setEditingPreset({...editingPreset, focusKeywords: e.target.value})}
                                            className="w-full bg-gray-900 border border-gray-600 rounded-md px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-y"
                                            rows={2}
                                        />
                                    </div>
                                    ...
                                    <div className="flex-grow overflow-hidden">
                                        <p className="font-semibold truncate text-gray-200" title={preset.name}>{preset.name}</p>
                                        <p className="text-sm text-gray-400 truncate" title={preset.avoidKeywords}><b>Tránh:</b> {preset.avoidKeywords || '(không có)'}</p>
                                        <p className="text-sm text-gray-400 truncate" title={preset.focusKeywords}><b>Nhấn mạnh:</b> {preset.focusKeywords || '(không có)'}</p>
                                    </div>
                                )}
                                <div className="flex flex-shrink-0 ml-2">
                                    {editingPreset?.id === preset.id ? (
                                        <>
                                            <button onClick={handleUpdatePreset} className="p-2 rounded-full hover:bg-gray-600 text-green-400" title="Save Preset">
                                                <SaveIcon className="w-5 h-5" />
                                            </button>
                                            <button onClick={handleCancelEdit} className="p-2 rounded-full hover:bg-gray-600 text-gray-300" title="Cancel Edit">
                                                <CloseIcon className="w-5 h-5" />
                                            </button>
                                        </>
                                    ) : (
                                        <button onClick={() => handleStartEdit(preset)} className="p-2 rounded-full hover:bg-gray-600 text-gray-300 hover:text-white" title="Edit Preset">
                                            <EditIcon className="w-5 h-5" />
                                        </button>
                                    )}
                                    <button onClick={() => handleDeletePreset(preset.id)} className="p-2 rounded-full hover:bg-gray-600 text-red-400 hover:text-red-300" title="Delete Preset">
