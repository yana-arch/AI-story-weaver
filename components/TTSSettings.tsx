...
export const TTSSettings: React.FC<TTSSettingsProps> = ({
  ...
  const googleVoices: VoiceOption[] = [
    ...
    {
      name: 'Google Tiếng Việt Nam (Nữ)',
      voiceURI: 'Google Tiếng Việt Nam',
      lang: 'vi-VN',
      gender: 'female',
      type: 'google',
    },
    {
      name: 'Google Tiếng Việt (Nữ)',
      voiceURI: 'Google Tiếng Việt',
      lang: 'vi-VN',
      gender: 'female',
      type: 'google',
    },
    ...
    },
    {
      ...
      voiceURI: 'Google Français Male',
      lang: 'fr-FR',
      gender: 'male',
      type: 'google',
    },
    {
      name: 'Google Deutsch (Frau)',
    ...
    {
      name: 'Google Deutsch (Mann)',
      voiceURI: 'Google Deutsch Male',
      lang: 'de-DE',
      gender: 'male',
      type: 'google',
    },
    {
      name: 'Google Español (Mujer)',
    ...
    {
      name: 'Google Español (Hombre)',
      voiceURI: 'Google Español Male',
      lang: 'es-ES',
      gender: 'male',
      type: 'google',
    },
  ...
  const handleTestVoice = () => {
    if ('speechSynthesis' in window) {
      ...
      const isVietnamese =
        localSettings.voiceURI?.includes('vi') ||
        localSettings.voiceURI?.includes('Viet') ||
        localSettings.voiceURI?.includes('Tiếng Việt');
      ...
      if (localSettings.voiceURI) {
        ...
        if (!selectedVoice && localSettings.voiceURI!.includes('Google')) {
          const isMale =
            localSettings.voiceURI!.includes('Male') ||
            localSettings.voiceURI!.includes('Homme') ||
            localSettings.voiceURI!.includes('Mann') ||
            localSettings.voiceURI!.includes('Hombre') ||
            localSettings.voiceURI!.includes('남성') ||
            localSettings.voiceURI!.includes('男性');
          const lang = localSettings.voiceURI!.includes('vi')
            ? 'vi'
            : localSettings.voiceURI!.includes('en-US')
              ? 'en-US'
              : localSettings.voiceURI!.includes('en-GB')
                ? 'en-GB'
                : localSettings.voiceURI!.includes('fr')
                  ? 'fr'
                  : localSettings.voiceURI!.includes('de')
                    ? 'de'
                    : localSettings.voiceURI!.includes('es')
                      ? 'es'
                      : localSettings.voiceURI!.includes('ja')
                        ? 'ja'
                        : localSettings.voiceURI!.includes('ko')
                          ? 'ko'
                          : '';
        ...
        <div className="space-y-4">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setLocalSettings((prev) => ({ ...prev, voiceURI: e.target.value || undefined }))
              }
            ...
            <div className="mt-2 space-y-1">
              ...
              {voices.filter((v) => v.lang.startsWith('vi')).length > 0 && (
                <p className="text-xs text-green-500">
                  ✓ Đã tìm thấy {voices.filter((v) => v.lang.startsWith('vi')).length} giọng nói
                  tiếng Việt
                </p>
              )}
              <div className="text-xs text-muted-foreground">
                <p>
                  💡 <strong>Google Voices:</strong> Giọng nói chất lượng cao, tự nhiên
                </p>
                <p>
                  💻 <strong>System Voices:</strong> Giọng nói có sẵn trên thiết bị
                </p>
              </div>
            </div>
          </div>
