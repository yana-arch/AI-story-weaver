...
import {
  EditIcon, SaveIcon, CloseIcon, SpeakerIcon,
  HistoryIcon, CopyIcon, TrashIcon, DragHandleIcon
} from './icons';
...
export const StorySegmentList = memo(({
  ...
  const renderSegment = useCallback((segment: StorySegment, index: number) => {
      ...
      >
        ...
        <div className={`relative p-5 rounded-lg mb-6 ${segment.type === 'user' ? 'bg-secondary' : 'bg-primary/10 border border-primary/20'}`}>
          ...
          <div className="absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            {editingSegmentId === segment.id ? (
              <>
                <button onClick={handleSaveEdit} className="p-1.5 rounded hover:bg-secondary/80 text-green-400" title="Save">
              ...
              <>
                ...
                <button onClick={() => handleStartEdit(segment)} className="p-1.5 rounded hover:bg-secondary/80" title="Edit">
            ...
            <button onClick={() => handleCopyContent(segment.content)} className="p-1.5 rounded hover:bg-secondary/80" title="Copy">
              <CopyIcon className="w-4 h-4" />
            </button>
            <button onClick={() => handleDeleteSegment(segment.id)} className="p-1.5 rounded hover:bg-secondary/80 text-red-400" title="Delete">
