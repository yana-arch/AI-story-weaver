    ...
    <div className="fixed inset-0 bg-background/80 flex justify-center items-center z-50">
      <div className="bg-card rounded-lg shadow-2xl p-6 w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-3 mb-4 bg-muted/50 rounded-lg">
          <div>
            <h3 className="font-semibold text-foreground">
              {isApiKeyAvailable
                ? 'Sử dụng API Key mặc định của Gemini'
                : 'API Key mặc định không khả dụng'}
            </h3>
            <p className="text-xs text-muted-foreground">
              {isApiKeyAvailable
                ? 'Sử dụng key được cung cấp sẵn (chia sẻ, có thể bị giới hạn).'
                : 'Vui lòng thiết lập API_KEY trong file biến môi trường.'}
            </p>
          </div>
          <label
            htmlFor="default-key-toggle"
            className={`relative inline-flex items-center ${isApiKeyAvailable ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'}`}
          >
            <input
              type="checkbox"
              title={isApiKeyAvailable ? 'Toggle default API key' : 'API key not available'}
              disabled={!isApiKeyAvailable}
              checked={useDefaultKey && isApiKeyAvailable}
              onChange={(e) => isApiKeyAvailable && setUseDefaultKey(e.target.checked)}
              id="default-key-toggle"
              className="sr-only peer"
            />
            <div
              className={`w-11 h-6 rounded-full peer peer-focus:ring-2 peer-focus:ring-offset-2 peer-focus:ring-offset-background peer-focus:ring-ring peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all ${isApiKeyAvailable ? 'bg-muted peer-checked:bg-primary' : 'bg-red-300'}`}
            ></div>
          </label>
        </div>
        ...
        <div className="space-y-3 mb-4 p-4 border border-border rounded-lg">
          ...
          <input
            type="text"
            placeholder="Tên nhà cung cấp (ví dụ: 'Gemini chính')"
            value={newKeyName}
          ...
          />
          <textarea
            placeholder="API Key * (có thể nhập nhiều key cách nhau bởi ',' hoặc xuống dòng hoặc dấu cách)"
            rows={3}
            value={newKeyValue}
            onChange={(e) => setNewKeyValue(e.target.value)}
            className="w-full bg-input border border-border rounded-md px-3 py-2 text-foreground focus:outline-none focus:ring-2 focus:ring-ring font-mono text-sm"
          />
          <p className="text-xs text-muted-foreground -mt-2 pl-1">
            Tùy chọn cho nhà cung cấp khác:
          </p>
          <input
            type="text"
            placeholder="Endpoint (Base URL) (e.g., https://openrouter.ai/api/v1)"
            value={newEndpoint}
          ...
          <input
            type="text"
            placeholder="Model ID (e.g., google/gemini-flash-1.5)"
            value={newModelId}
                    ...
                    <div className="flex items-center flex-grow overflow-hidden">
                      ...
                      <div className="overflow-hidden flex-grow">
                        {editingProvider === apiKey.id ? (
                          <div className="flex flex-col space-y-2 pr-2">
                            ...
                            <div className="flex space-x-2">
                              ...
                              <button
                                ...
                                className="px-3 py-1 bg-muted text-foreground text-xs rounded hover:bg-muted/80"
                                title="Cancel"
                              >
                                Cancel
                              </button>
                          ...
                          <div>
                            <p className="font-semibold truncate" title={apiKey.name}>
                              {apiKey.name}
                            </p>
                            {!apiKey.isDefault && apiKey.endpoint && apiKey.modelId ? (
                              <p
                                className="text-xs text-muted-foreground font-mono truncate"
                                title={apiKey.modelId}
                              >
                                {apiKey.modelId} @ {apiKey.endpoint}
                              </p>
                            ) : !apiKey.isDefault ? (
                    ...
                    {!apiKey.isDefault && (
                      <div className="flex items-center flex-shrink-0 ml-2">
                        <div className="w-6 h-6 flex items-center justify-center mr-1">
                          ...
                          {testStatus[apiKey.id] === 'success' && (
                            <span title="Test successful!">
                              <CheckCircleIcon className="w-5 h-5 text-green-500" />
                            </span>
                          )}
                          {testStatus[apiKey.id] === 'error' && (
                            <span title={testError[apiKey.id] || 'Test failed!'}>
                        ...
                        <button
                          ...
                          className="p-2 rounded-full hover:bg-muted text-muted-foreground hover:text-foreground disabled:text-muted disabled:cursor-not-allowed"
                          title="Test Key"
                        >
                        ...
                        <button
                          ...
                          className="p-2 rounded-full hover:bg-muted text-destructive hover:text-destructive/80"
                          title="Delete Key"
                        >
