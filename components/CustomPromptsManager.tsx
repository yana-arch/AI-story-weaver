...
export const CustomPromptsManager: React.FC<CustomPromptsManagerProps> = ({
  prompts,
  setPrompts,
  onClose,
}) => {
              ...
              {editingPrompt ? (
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-muted text-foreground font-semibold rounded-md hover:bg-muted/80 transition-colors"
                  >
                    Hủy
                  </button>
                </div>
              ...
              <ul className="space-y-2">
                {prompts.length > 0 ? (
                  prompts.map((prompt) => (
                    <li
                      key={prompt.id}
                      className="p-3 rounded-md bg-muted/50 flex justify-between items-start"
                    >
                      <div className="overflow-hidden">
                        <p className="font-semibold truncate text-foreground" title={prompt.title}>
                          {prompt.title}
                        </p>
                        <p
