...
const Section: React.FC<{ title: string; children: React.ReactNode; initialOpen?: boolean }> = ({
  title,
  children,
  initialOpen = true,
}) => {
          ...
          {config.generationMode === GenerationMode.REWRITE && (
            <div className="mt-3 space-y-3">
              ...
              >
                ...
                <option value={RewriteTarget.SELECTED_CHAPTER}>
                  {RewriteTarget.SELECTED_CHAPTER}
                </option>
              </LabeledSelect>
...
Ví dụ: Một chapter truyện với ngôn ngữ thơ mộng, tập trung vào cảm xúc lãng mạn và các cảnh gợi cảm tinh tế...`}
        ...
        <Section title="Cấu trúc tường thuật" initialOpen={true}>
          <ComboboxInput
            ...
            onChange={(e) =>
              handleChange('narrativeStructure', e.target.value as NarrativeStructure)
            }
        ...
        <Section title="Yêu cầu tùy chỉnh">
          <div className="flex gap-2">
            <button
              ...
              className="w-full flex justify-center items-center px-3 py-2 text-sm bg-secondary font-semibold rounded-md hover:bg-secondary/80 transition-colors"
            >
              ...
              customPrompts.map((prompt) => (
                ...
                >
                  <input
                    ...
                    id={`prompt-${prompt.id}`}
                    checked={
                      Array.isArray(selectedPromptIds) && selectedPromptIds.includes(prompt.id)
                    }
        ...
        <Section title="Bộ lọc và Trọng tâm từ khóa">
          ...
          <div className="flex gap-2">
            <button
              onClick={onSaveKeywordPreset}
              className="w-full flex justify-center items-center px-3 py-2 text-sm bg-primary/80 text-primary-foreground font-semibold rounded-md hover:bg-primary transition-colors"
            >
              <SaveIcon className="w-4 h-4 mr-2" />
              Lưu làm mẫu
            </button>
            <button
              onClick={onManageKeywordPresets}
              className="w-full flex justify-center items-center px-3 py-2 text-sm bg-secondary font-semibold rounded-md hover:bg-secondary/80 transition-colors"
            >
              Quản lý mẫu
            </button>
        ...
        <Section title="Tùy chọn Nội dung 18+">
          <p className="text-xs text-muted-foreground -mt-2 mb-2">
            Chọn các yếu tố cần tập trung. Bỏ trống để giữ SFW.
          </p>
          <div className="space-y-2">
            {Object.values(AdultContentOptions).map((option) => (
              <label
                key={option}
                htmlFor={`adult-${option}`}
                className="flex items-center p-2 rounded-md hover:bg-muted/50 cursor-pointer"
              >
                <input
                  type="checkbox"
