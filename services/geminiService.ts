...
async function parseJsonResponse<T>(
  ...
  const { maxRetries = 2, retryCallback, validateResult, defaultEmptyResult } = options;
  ...
  while (attempt <= maxRetries) {
    try {
      ...
      throw new Error(
        `Failed to parse JSON response after ${attempt + 1} attempts: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
...
export async function testApiKey(apiKey: ApiKey): Promise<void> {
  ...
  if (apiKey.endpoint && apiKey.modelId) {
    ...
    const effectiveApiKey = apiKey.id === 'default' ? process.env.API_KEY || '' : firstKey;
  ...
  const responseSchema = {
    ...
    items: {
      ...
      properties: {
        id: {
          type: Type.STRING,
          description: 'A unique identifier for the character, can be generated from the name.',
        },
  ...
  for (let i = 0; i < apiKeys.length; i++) {
    ...
    try {
      ...
      if (item.apiKey.endpoint && item.apiKey.modelId) {
        ...
        const parsedResult = await parseJsonResponse<any>(generatedText, {
          maxRetries: 2,
        ...
        const effectiveApiKey = item.apiKey.id === 'default' ? process.env.API_KEY || '' : item.key;
        if (!effectiveApiKey) {
      ...
      console.error(
        `API call for character profiles with key "${item.apiKey.name}" failed.`,
        error
      );
      keyIndex = (keyIndex + 1) % apiKeys.length;
    }
...
export async function generateStorySegment(
  ...
  if (!apiKeys || apiKeys.length === 0) {
    throw new Error(
      'Không có API key nào được cung cấp. Vui lòng thêm API key hoặc bật key mặc định trong phần cài đặt.'
    );
  }
  ...
  const customPromptsSection =
    customPromptsContent.length > 0
      ? `
--- YÊU CẦU TÙY CHỈNH ---
${customPromptsContent.map((p) => `- ${p}`).join('\n')}`
      : '';

  const adultContentSection =
    config.adultContentOptions.length > 0
      ? `
...
- Giữ cho câu chuyện phù hợp với mọi lứa tuổi. Tránh mọi nội dung người lớn.`;
  ...
  const characterProfilesSection =
...
${characterProfilesSection}`;
  ...
  throw new Error(
    'Tất cả các API key đều thất bại. Vui lòng kiểm tra lại key hoặc kết nối mạng của bạn.'
  );
}
